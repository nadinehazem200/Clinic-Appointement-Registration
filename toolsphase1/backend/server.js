    ...
    db.query(insertUserQuery, [username, password, userType], (insertErr, results) => {
      ...
      if (userType === 'doctor') {
        ...
        db.query(updateDoctorIdQuery, [insertedUserId], (updateErr) => {
          ...
          return res.status(201).json({ message: `User and doctor created successfully. Your user id is: ${insertedUserId}` });
        });
        ...
        db.query(updatePatientIdQuery, [insertedUserId], (updateErr) => {
          ...
          return res.status(201).json({ message: `User and patient created successfully. Your user id is: ${insertedUserId}` });
        });
      } else {
        return res.status(201).json({ message: `User created successfully. Your user id is: ${insertedUserId}` });
      }
    });
  });
});
...
// Endpoint to cancel a patient's appointment
app.delete('/appointments/:appID', (req, res) => {
  ...
  db.query('DELETE FROM appointments WHERE appID = ?', [appID], (err, results) => {
    if (err) {
...
app.get('/patients/:patientID/reservations', (req, res) => {
